...
function catpos($catid, $symbol=' > '){
	...
	foreach($arrparentid as $catid) {
		...
		$pos .= '<a href="'.$url.'">'.$category_arr[$catid]['catname'].$symbol.'</a>';
	}
	return $pos;
}
/**
...
function get_sql_catids($catids)
{
     return " AND `catid` IN(".$catids.") ";
    /*
    if(stripos($catid,'|'))
    {
        $arrcatid = explode('|',$catid);
        $arrchildids = '';
        foreach((array)$arrcatid AS $k => $c)
        {
            if($c)
            {
                $arrchildids .= $CATEGORY[$catid]['arrchildid'].",";
            }
        }
        $arrchildids = substr($arrchildids,0,-1);
        return " AND `catid` IN(".$arrchildids.") ";
    }
    else
    {
        $catid = intval($catid);
